...
$sql_amount = "SELECT COUNT(billno) as billno,CONCAT('Rs. ', FORMAT(sum(amount), 2, 'en_IN'))  as amount FROM tbl_pos_date_sales
...
$sql="SELECT DATE_FORMAT(bill_date,'%M %Y') AS bill_date,sum(amount) as amount  FROM tbl_pos_date_sales
WHERE client_id=".$data1->clientid." AND bill_date between  DATE_FORMAT(STR_TO_DATE('".$fromdate."', '%Y-%m-%d'), '%Y-%m-%d') and DATE_FORMAT(STR_TO_DATE('".$todate."', '%Y-%m-%d'), '%Y-%m-%d')
GROUP BY DATE_FORMAT(bill_date,'%M %Y') ORDER BY bill_date DESC";
